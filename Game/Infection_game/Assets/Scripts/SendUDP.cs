...
public class SendUDP : MonoBehaviour {
	public int listenPort = 2003; // ポートはサーバ・クライアントで合わせる必要がある
	...
	void Start(){
		maxX = (int)Camera.main.ViewportToScreenPoint(new Vector2 (1, 1)).x;
		maxY = (int)Camera.main.ViewportToScreenPoint(new Vector2 (1, 1)).y;
	}
	...
	void Update() {
		if (coroutine_ == null && Input.GetKeyDown(KeyCode.Space)) {
			string sendMsg = "x: "+ maxX.ToString() +" y: " + maxY.ToString();	 // 任意のテキスト（長すぎないこと）
